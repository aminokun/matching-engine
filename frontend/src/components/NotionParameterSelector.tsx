...
export default function NotionParameterSelector({
  onParametersChange,
}: NotionParameterSelectorProps) {
    ...
    const fetchDatabases = async () => {
      ...
      try {
        ...
        if (data.databases?.length === 0) {
          setError(
            "No Notion databases found. Make sure NOTION_API_KEY is configured in the backend."
          );
        }
      } catch (err) {
        console.error("Failed to fetch databases:", err);
        setError(
          "Could not connect to Notion. Please configure NOTION_API_KEY in backend environment."
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleAddDatabase = async (database: NotionDatabase) => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/notion/database/${database.id}`,
        {
          method: "GET",
        }
      );
    ...
    <div className="mb-6">
              ...
              {databases.length === 0 ? (
                ...
                databases.map((db) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3  text-sm text-gray-800 flex items-center gap-2 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed border-b last:border-b-0 ${
      ...
      {selectedDatabases.length > 0 ? (
            ...
            >
              <div className="flex-1">
                ...
                <p className="text-xs text-blue-700">
                  {db.parameters.length}{" "}
                  {db.parameters.length === 1 ? "option" : "options"}
                </p>
              </div>
              <button
        ...
        <div className="p-4 text-sm text-gray-600 bg-gray-50 rounded-lg border border-gray-200 text-center">
          No parameter sets selected yet. Click "Add Parameter Set" to get
          started.
        </div>
      )}
