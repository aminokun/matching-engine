        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-green-700">
              Search completed successfully! Found {response.totalHits || response.total_results || 0} companies in {response.took ? `${response.took}ms` : response.search_time || "unknown time"}.
            </p>
          </div>
        </div>
      </div>
      ...
      {response.results && Array.isArray(response.results) && response.results.length > 0 ? (
        <div className="space-y-4">
          {response.results.map((result: any, index: number) => {
            const company = result.companyDetails || result.company || result;
            const classification = result.classification || {};
            const explanation = response.explanations?.[result.profileId] || result.match_reasons?.join(' ') || '';
                ...
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">
                      {company.companyName || company.company_name || "Unknown Company"}
                    </h3>
